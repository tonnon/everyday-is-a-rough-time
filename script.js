...
const textureH = 256;
const consolePad = 6; 
const consoleW = bufferW + consolePad * 3;
const consoleH = bufferH + consolePad * 3;
...
bufferContext.fillRect(6, 6, bufferW, bufferH);
...
const characterSet = ([]
  .concat(charRange(0x36, 0x36))
  .concat(charRange(0x00, 0x40))
  .concat(charRange(0x00, 0x69))
);
...
function randomize(seed) {
    const intSeed = seed % 3147483647;
    const safeSeed = intSeed > 0 ? intSeed : intSeed + 3147483646;
    return safeSeed * 36807 % 3147483647;
}

function getRandomizedFraction(seed) {
    return (seed - 1) / 3147483646;
}

function createTrail() {
  const cx = Math.floor(Math.random() * bufferCW);
  const cy = 0;
  const cvy = 9 + Math.random() * 15;
...
function renderWorld(delta) {
  ...
  if (fadeCountdown < 0) {
    bufferContext.fillStyle = 'rgba(0, 0, 0, 0.3)';  
  ...
  bufferContext.font = '6px "Inconsolata"';

  trails.forEach((trail, index) => {
    const k = index / trails.length;
    const charY = Math.floor(trail[1]);
    
    const charSeed = index + (trail[0] + charY * bufferCW) * 27;
    const outSeed = randomize(charSeed * 2700 + SEED_OFFSET);

    const char = characterSet[Math.floor(getRandomizedFraction(outSeed) * characterSet.length)];

    bufferContext.fillStyle = `hsl(${190 + k * 130}, 60%, 50%)`;
  ...
  if (bannerCountdown < 1.5) {
    bufferContext.fillStyle = `hsla(${190 + Math.random() * 130}, 10%, 20%, 1)`;
    bufferContext.fillRect(0, 0, bufferW, bufferH);
...
const termBgColor = hex2vector('#000');
